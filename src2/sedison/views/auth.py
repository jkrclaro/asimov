...
from sedison.sedison.helpers.email import (
    email_confirmation,
    email_change_email_success,
    email_reset,
    email_reset_success
)
from sedison.sedison.helpers.recaptcha import validate_recaptcha
...
def reset():
    ...
    if form.validate_on_submit():
        ...
        email_reset_success()
        login_user(user)
        flash('Successfully changed your Sedison password', 'success')
        return redirect(url_for('merchant.index'))
