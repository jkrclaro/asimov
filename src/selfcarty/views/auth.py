...
from src.selfcarty.models.merchant import Merchant
from src.selfcarty.forms import (
    SignupForm,
    LoginForm,
    ForgotPasswordForm,
    ResetPasswordForm,
...
from src.selfcarty.helpers.email import (
    email_confirmation,
    email_change_email_success,
    email_reset,
    email_reset_success
)
from src.selfcarty.helpers.recaptcha import validate_recaptcha
...
def reset():
    ...
    if form.validate_on_submit():
        ...
        email_reset_success()
        login_user(user)
        flash('Successfully changed your Selfcarty password', 'success')
        return redirect(url_for('merchant.index'))
