...
from src.spidxr.models.auth import User, Profile
from src.spidxr.models.merchant import Merchant
from src.spidxr.forms import (
    SignupForm,
    LoginForm,
    ForgotPasswordForm,
    ResetPasswordForm,
...
from src.spidxr.helpers.email import (
    email_confirmation,
    email_change_email_success,
    email_reset,
    email_reset_success
)
from src.spidxr.helpers.recaptcha import validate_recaptcha
...
def reset():
    ...
    if form.validate_on_submit():
        ...
        email_reset_success()
        login_user(user)
        flash('Successfully changed your Spidxr password', 'success')
        return redirect(url_for('dashboard.index'))
