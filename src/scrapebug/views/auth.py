...
from src.scrapebug.models.auth import User, Profile
from src.scrapebug.models.merchant import Merchant
...
from src.scrapebug.helpers.email import (
    email_confirmation,
    email_change_email_success,
    email_reset,
    email_reset_success
)
from src.scrapebug.helpers.recaptcha import validate_recaptcha
...
def reset():
    ...
    if form.validate_on_submit():
        ...
        email_reset_success()
        login_user(user)
        flash('Successfully changed your Scrapebug password', 'success')
        return redirect(url_for('dashboard.index'))
