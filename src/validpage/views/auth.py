...
from src.validpage.models.merchant import Merchant
from src.validpage.forms import (
    SignupForm,
    LoginForm,
    ForgotPasswordForm,
    ResetPasswordForm,
...
from src.validpage.helpers.email import (
    email_confirmation,
    email_change_email_success,
    email_reset,
    email_reset_success
)
from src.validpage.helpers.recaptcha import validate_recaptcha
...
def reset():
    ...
    if form.validate_on_submit():
        ...
        email_reset_success()
        login_user(user)
        flash('Successfully changed your Validpage password', 'success')
        return redirect(url_for('merchant.index'))
