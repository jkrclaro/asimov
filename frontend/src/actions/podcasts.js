...
export const getPodcasts = () => async (dispatch, getState) => {
    ...
    const response = await pxdcast.get('/subscriptions', tokenConfig(getState));
    dispatch({
...
export const fetchEpisodes = (id) => async dispatch => {
    ...
    try {
        const response = await pxdcast.get(`/podcasts/${id}/episodes/`)
        dispatch({
...
export const getPodcast = id => async (dispatch, getState) => {
    ...
    try {
        const response = await pxdcast.get(`/podcasts/${id}`, tokenConfig(getState));
        dispatch({
...
export const subscribePodcast = name => async (dispatch, getState) => {
    const payload = { name }
    await pxdcast.post('/podcasts/subscribe', payload, tokenConfig(getState));
    dispatch({
...
export const unsubscribePodcast = name => async (dispatch, getState) => {
    const payload = { name }
    await pxdcast.post('/podcasts/unsubscribe', payload, tokenConfig(getState));
    dispatch({
...
export const getPodcastSubscription = itunes_id => async (dispatch, getState) => {
    ...
    const response = await pxdcast.post('/podcasts/subscription', payload, tokenConfig(getState));
    dispatch({
